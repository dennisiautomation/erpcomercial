...
⚠️ **DEPLOY PENDENTE** — Dennis decide quando. Sem migration; toca só
`app/Http/Controllers/App/ConfiguracaoLojaController.php` e
`resources/views/app/configuracoes/edit.blade.php`, então é o rito de sempre **sem rebuild**
(tar de `app` + `resources` → `artisan optimize` → `chown www-data` → `kill -USR2` no master do
php-fpm, **não** o PID 1 — armadilhas 34 e 26b).
