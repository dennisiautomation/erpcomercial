...
`type="text" inputmode="decimal"` — **sem spinner, a roda do mouse não tem no que mexer**, e o
teclado do celular continua numérico. O resto vira JS na própria view:

- digitação aceita só dígito e **um** separador; ponto vira vírgula; **sinal de menos não entra**;
- no submit, vírgula vira ponto (`1,5` → `1.5`) e o que sobra inválido fica **marcado em vermelho
  na célula**, com o scroll indo até ela e um toast — em vez do balão nativo que trava sem explicar;
- o servidor continua validando `numeric|min:0` (testado: POST forjado com `-5` **não grava**).

O valor exibido passou a sair em pt-BR (`number_format(..., ',', '')`), casando com o que o usuário
digita.

📌 **Fração continua permitida** (a coluna é `decimal(12,3)` e há ERP de outro ramo na plataforma) —
o que sumiu foi o jeito de ela aparecer sem ninguém pedir. Travar em inteiro é decisão do Dennis.

⚠️ **O mesmo risco existe em toda tela com `type="number"`** — movimentações, transferências,
orçamentos, pedidos, OS, etiquetas. O antídoto de uma linha é global (`wheel` → `blur` em
`input[type=number]` no `erp-core.js`), mas `public/` **não vai no tar do deploy** (armadilha 46):
exige rebuild ou `docker cp` próprio. Fora desta entrega.

### Validação (`erp-test-app`)

Lógica do campo exercitada em node com um DOM de mentira: `13`→`13`, `13.5`→`13,5`, `-5`→`5`,
`1,2,3`→`1,23`, `abc`→vazio; no submit `13,5`→`13.5` e `-5`/`abc` **marcam a célula e bloqueiam**.
POST real gravou 13 (inteiro) e 1,5 (fracionado) na movimentação; POST com `-5` foi barrado pelo
servidor e o saldo ficou intacto.
...
66. **`<input type="number">` numa tabela que rola é estoque corrompido esperando a hora.** O
    ...
    quantidade, use **`type="text" inputmode="decimal"`** e normalize vírgula→ponto no submit —
    teclado numérico no celular, zero spinner. Vale para qualquer campo numérico em tabela larga.
