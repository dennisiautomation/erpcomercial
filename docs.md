...
51b. **O fuso do app era UTC até 25/08/2026 — datas antigas em backup/XML/log são UTC.**
    `config/app.php` nasceu com `timezone => 'UTC'` e todo o histórico foi gravado 3h à frente
    da hora local. Em 25/08 o app virou `America/Sao_Paulo` (env `APP_TIMEZONE`, default BR no
    config) e o banco recebeu data-fix de -3h em todas as colunas datetime de negócio (178
    colunas/65 tabelas; script e backup em `/home/ubuntu/erp-backups/`). Consequências: (a) o
    dump `pre-fuso-fix-20260825.sql.gz` e anteriores estão em UTC — restaurar um deles exige
    reaplicar o shift; (b) `laravel.log` antigo é UTC, o novo é local; (c) os crons `dailyAt`
    do scheduler passaram a disparar em hora LOCAL (backup 3h, cobranças 6h, certificado 8h —
    antes rodavam 3h mais cedo no relógio local); (d) o MySQL ganhou `--default-time-zone=-03:00`
    (SET GLOBAL ao vivo + compose p/ recreate) por causa dos `CURDATE()` no RelatorioController.
    Ao criar app novo, definir o fuso no dia 1 — data-fix retroativo é cirurgia.
