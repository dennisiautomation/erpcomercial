...
### A célula suja fica visível, mas ninguém a limpa por baixo

Dennis decidiu **não** rodar limpeza em massa (04/09) — a loja corrige na contagem. Então:

- a célula com fração é exibida **com o valor real** e pintada de **âmbar**, com `title` explicando
  de onde veio. Arredondar na tela seria pior que o problema: a próxima gravação viraria uma
  limpeza em massa silenciosa;
- ela carrega `data-fracionario` e o submit **a ignora enquanto ninguém a tocar** — abrir a tela e
  salvar não gera movimentação nenhuma nesses produtos (testado: saldo `1,5` intocado, zero
  movimentação nova). Digitou por cima, a marca cai e o valor vira o que a pessoa contou;
- o rodapé só mostra o aviso quando há célula assim na página.
...
66. **`<input type="number">` numa tabela que rola é estoque corrompido esperando a hora.** O
    ...
    spinner — e trate vírgula cortando o resto (`13,5` → `13`, nunca `135`). O antídoto global está
