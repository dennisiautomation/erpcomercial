...
9p. [Vendedor opera somente o PDV (04/09/2026)](#vendedor-opera-somente-o-pdv-04092026)
...
Branch `feat/vendedor-apenas-pdv`. Pedido do Dennis: "uma configuração que, se ativa, o perfil
vendedor só vê o menu PDV e nenhum outro — configurável por cliente, e vem por padrão desativado".
...
2. **Perfil `consulta`** — os 4 usuários assim da N S BORBA continuam vendo o menu inteiro. Ficou
   fora por decisão dele; estender exige destino próprio (consulta não tem `vendas,criar`).
3. **Fazer o menu inteiro perguntar à matriz** (armadilha 65 nos 8 blocos) — fecha os 14 links que
   dão 403 para o vendedor de quem NÃO ligou a chave. Muda tela de quem não pediu: precisa de OK.
4. Apagar `qa.vendedor@teste.local` e `qa.vendedor6@teste.local` e refazer a senha do
