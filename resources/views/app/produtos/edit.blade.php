...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <small class="text-muted">{{ $produto->descricao }} <code class="ms-1">{{ $produto->codigo_interno }}</code></small>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('app.produtos.show', $produto) }}" class="btn btn-outline-info">
...
<form method="POST" action="{{ route('app.produtos.update', $produto) }}" enctype="multipart/form-data" id="formProduto" novalidate>
    @csrf
    @method('PUT')
        ...
        <div class="card shadow-sm border-0">
            <div class="card-body p-4">
                <div class="text-center mb-4">
                    ...
                    <p class="text-muted mb-0">Dados basicos do produto</p>
                </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" name="descricao" id="descricao" class="form-control form-control-lg @error('descricao') is-invalid @enderror" value="{{ old('descricao', $produto->descricao) }}" placeholder="Ex: Camiseta Polo Azul M" required>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="descricao_detalhada" id="descricao_detalhada" class="form-control @error('descricao_detalhada') is-invalid @enderror" rows="2" placeholder="Detalhes adicionais do produto">{{ old('descricao_detalhada', $produto->descricao_detalhada) }}</textarea>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="hidden" name="unidade_medida" id="unidade_medida" value="{{ old('unidade_medida', $produto->unidade_medida) }}" required>
                        <div class="d-flex flex-wrap gap-2" id="unidadeCards">
                            @foreach(['UN' => 'Unidade', 'KG' => 'Quilo', 'CX' => 'Caixa', 'PCT' => 'Pacote', 'LT' => 'Litro', 'MT' => 'Metro', 'M2' => 'M2', 'M3' => 'M3', 'PAR' => 'Par', 'JG' => 'Jogo'] as $val => $label)
                                <div class="unidade-card {{ old('unidade_medida', $produto->unidade_medida) === $val ? 'selected' : '' }}" data-value="{{ $val }}">
                                    <strong>{{ $val }}</strong>
                                    <small>{{ $label }}</small>
                                </div>
                            @endforeach
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            <input type="number" name="markup" id="markup" class="form-control bg-light" value="{{ old('markup', $produto->markup) }}" step="0.01" min="0" readonly>
                            <span class="input-group-text">%</span>
                        </div>
                    </div>

                    {{-- Preço no cartão (opcional — vale para crédito e débito) --}}
                    @php
                        $precosForma = $produto->precos->pluck('valor', 'modalidade');
                        $precoCartaoAtual = $precosForma['credito'] ?? $precosForma['debito'] ?? '';
                    @endphp
                    <div class="col-12">
                        <div class="border rounded p-3 bg-light-subtle">
                            <div class="fw-semibold mb-1"><i class="bi bi-credit-card me-1"></i> Preço no Cartão <span class="text-muted fw-normal">(opcional)</span></div>
                            <div class="text-muted small mb-3">Vale para <strong>crédito e débito</strong>. O preço à vista acima vale para Dinheiro/PIX. Deixe em branco para usar a regra geral das <a href="{{ route('app.configuracoes.edit') }}" target="_blank">Configurações da Loja</a>.</div>
                            <div class="row g-3">
                                <div class="col-md-6">
    ...
    <div class="wizard-step" id="wizardStep2">
        <div class="card shadow-sm border-0">
            <div class="card-body p-4">
                ...
                <div class="row g-3">
                    {{-- Codigo Interno (readonly) --}}
                    <div class="col-md-4">
                        <label class="form-label text-muted">Codigo interno</label>
                        <input type="text" class="form-control bg-light fw-bold" value="{{ $produto->codigo_interno }}" readonly disabled>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="codigo_barras" id="codigo_barras" class="form-control @error('codigo_barras') is-invalid @enderror" value="{{ old('codigo_barras', $produto->codigo_barras) }}" placeholder="Escaneie com o leitor ou digite o EAN">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="sku" id="sku" class="form-control @error('sku') is-invalid @enderror" value="{{ old('sku', $produto->sku) }}" placeholder="Ex: CAM-AZU-M">
                    ...
                    <div class="col-md-6">
                        ...
                        @if($produto->foto)
                            <div class="mt-2">
                                <img src="{{ Storage::url($produto->foto) }}" class="rounded border" style="max-height:80px;" alt="Foto atual">
                                <small class="d-block text-muted mt-1">Foto atual. Envie uma nova para substituir.</small>
                            </div>
                        @endif
            ...
            <div class="card-body p-4">
                ...
                @else
                    ...
                    <div class="alert alert-info d-flex align-items-start mb-4">
                        ...
                        <div>
                            <strong>Regime {{ str_replace('_', ' ', $regimeValue) }}.</strong><br>
                            <small class="text-muted">{{ $fiscalDefaults['help'] }} Revise os valores se necessario.</small>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="hidden" name="ncm" id="ncm" value="{{ old('ncm', $produto->ncm) }}">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="cfop" id="cfop" class="form-select @error('cfop') is-invalid @enderror">
                                ...
                                @foreach($cfopOptions as $code => $label)
                                    <option value="{{ $code }}" {{ old('cfop', $produto->cfop) == $code ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="cst_csosn" id="cst_csosn" class="form-select @error('cst_csosn') is-invalid @enderror">
                                <option value="">Selecione...</option>
                                <option value="102" {{ old('cst_csosn', $produto->cst_csosn) == '102' ? 'selected' : '' }}>102 - Tributada sem credito</option>
                                <option value="103" {{ old('cst_csosn', $produto->cst_csosn) == '103' ? 'selected' : '' }}>103 - Isenta de ICMS</option>
                                <option value="300" {{ old('cst_csosn', $produto->cst_csosn) == '300' ? 'selected' : '' }}>300 - Imune</option>
                                <option value="400" {{ old('cst_csosn', $produto->cst_csosn) == '400' ? 'selected' : '' }}>400 - Nao tributada</option>
                                <option value="500" {{ old('cst_csosn', $produto->cst_csosn) == '500' ? 'selected' : '' }}>500 - ICMS cobrado por ST</option>
                            </select>
                            @else
                            <label for="cst_csosn" class="form-label fw-semibold">
                            ...
                            <select name="cst_csosn" id="cst_csosn" class="form-select @error('cst_csosn') is-invalid @enderror">
                                <option value="">Selecione...</option>
                                <option value="00" {{ old('cst_csosn', $produto->cst_csosn) == '00' ? 'selected' : '' }}>00 - Tributada integralmente</option>
                                <option value="10" {{ old('cst_csosn', $produto->cst_csosn) == '10' ? 'selected' : '' }}>10 - Tributada com ST</option>
                                <option value="20" {{ old('cst_csosn', $produto->cst_csosn) == '20' ? 'selected' : '' }}>20 - Com reducao de base</option>
                                <option value="40" {{ old('cst_csosn', $produto->cst_csosn) == '40' ? 'selected' : '' }}>40 - Isenta</option>
                                <option value="41" {{ old('cst_csosn', $produto->cst_csosn) == '41' ? 'selected' : '' }}>41 - Nao tributada</option>
                                <option value="60" {{ old('cst_csosn', $produto->cst_csosn) == '60' ? 'selected' : '' }}>60 - ICMS cobrado por ST anterior</option>
                            </select>
                            @endif
                            ...
                            <div class="row g-3">
                                <div class="col-6 col-md-3">
                                    <div class="aliquota-card aliquota-icms">
                                        ...
                                        <div class="aliquota-card-body">
                                            <div class="input-group">
                                                <input type="number" name="icms_aliquota" id="icms_aliquota" class="form-control text-center fw-bold @error('icms_aliquota') is-invalid @enderror" value="{{ old('icms_aliquota', $produto->icms_aliquota) }}" step="0.01" min="0" max="100">
                                                <span class="input-group-text">%</span>
                                            </div>
                                        </div>
                                        <div class="aliquota-card-footer">
                                            <small>Imposto estadual</small>
                                        </div>
                                    </div>
                                ...
                                <div class="col-6 col-md-3">
                                    <div class="aliquota-card aliquota-pis">
                                        ...
                                        <div class="aliquota-card-body">
                                            <div class="input-group">
                                                <input type="number" name="pis_aliquota" id="pis_aliquota" class="form-control text-center fw-bold @error('pis_aliquota') is-invalid @enderror" value="{{ old('pis_aliquota', $produto->pis_aliquota) }}" step="0.01" min="0" max="100">
                                                <span class="input-group-text">%</span>
                                            </div>
                                        </div>
                                        <div class="aliquota-card-footer">
                                            <small>Contrib. federal</small>
                                        </div>
                                    </div>
                                ...
                                <div class="col-6 col-md-3">
                                    <div class="aliquota-card aliquota-cofins">
                                        ...
                                        <div class="aliquota-card-body">
                                            <div class="input-group">
                                                <input type="number" name="cofins_aliquota" id="cofins_aliquota" class="form-control text-center fw-bold @error('cofins_aliquota') is-invalid @enderror" value="{{ old('cofins_aliquota', $produto->cofins_aliquota) }}" step="0.01" min="0" max="100">
                                                <span class="input-group-text">%</span>
                                            </div>
                                        </div>
                                        <div class="aliquota-card-footer">
                                ...
                                <div class="col-6 col-md-3">
                                    <div class="aliquota-card aliquota-ipi">
                                        ...
                                        <div class="aliquota-card-body">
                                            <div class="input-group">
                                                <input type="number" name="ipi_aliquota" id="ipi_aliquota" class="form-control text-center fw-bold @error('ipi_aliquota') is-invalid @enderror" value="{{ old('ipi_aliquota', $produto->ipi_aliquota) }}" step="0.01" min="0" max="100">
                                                <span class="input-group-text">%</span>
                                            </div>
                                        </div>
                                        <div class="aliquota-card-footer">
                                            <small>Industrializados</small>
                                        </div>
                                    </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="origem" id="origem" class="form-select @error('origem') is-invalid @enderror">
                                ...
                                @foreach($origemOptions as $code => $label)
                                    <option value="{{ $code }}" {{ old('origem', $produto->origem) == (string)$code ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="cest" id="cest" class="form-control @error('cest') is-invalid @enderror" value="{{ old('cest', $produto->cest) }}" maxlength="10" placeholder="00.000.00">
