...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1"><i class="bi bi-pencil-square me-2"></i>Editar Orcamento #{{ $orcamento->numero }}</h4>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 small">
                <li class="breadcrumb-item"><a href="{{ route('app.dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ route('app.orcamentos.index') }}">Orcamentos</a></li>
                <li class="breadcrumb-item"><a href="{{ route('app.orcamentos.show', $orcamento) }}">#{{ $orcamento->numero }}</a></li>
    ...
    <div class="row g-4">
                        ...
                        <div id="clienteSelecionado" class="mt-2" style="{{ $orcamento->cliente ? '' : 'display:none' }}">
                            <div class="d-flex align-items-center bg-primary bg-opacity-10 rounded-3 p-2 ps-3">
                                ...
                                <div class="flex-grow-1">
                                    <div class="fw-semibold" id="clienteNome">{{ $orcamento->cliente->nome_razao_social ?? '' }}</div>
                                    <small class="text-muted" id="clienteDoc">{{ $orcamento->cliente->cpf_cnpj ?? '' }}</small>
                                </div>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <textarea name="observacoes_internas" class="form-control" rows="3"
                                  placeholder="Notas internas...">{{ $orcamento->observacoes_internas }}</textarea>
                    </div>
                    <div class="mb-0">
                        ...
                        <textarea name="observacoes_externas" class="form-control" rows="3"
                                  placeholder="Condicoes, prazo de entrega...">{{ $orcamento->observacoes_externas }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-6">
                        ...
                        <div class="col-6">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="desconto_percentual" id="descontoPerc"
                                       class="form-control" step="0.01" min="0" max="100" value="{{ $orcamento->desconto_percentual ?? 0 }}">
                        ...
                        <div class="col-6">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <input type="number" name="desconto_valor" id="descontoValor"
                                       class="form-control" step="0.01" min="0" value="{{ $orcamento->desconto_valor ?? 0 }}">
                            </div>
                        </div>
                    </div>
            ...
            <div class="mt-3 d-flex gap-2 justify-content-end">
                <a href="{{ route('app.orcamentos.show', $orcamento) }}" class="btn btn-outline-secondary btn-lg">
                    Cancelar
                </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @php
        $itensData = $orcamento->itens->map(fn($i) => [
