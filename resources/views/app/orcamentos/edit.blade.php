...
@section('title', 'Editar Orcamento #' . $orcamento->numero)

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><i class="bi bi-pencil-square me-2"></i>Editar Orcamento #{{ $orcamento->numero }}</h4>
    <a href="{{ route('app.orcamentos.show', $orcamento) }}" class="btn btn-outline-secondary">
        <i class="bi bi-arrow-left me-1"></i> Voltar
    </a>
</div>

<form method="POST" action="{{ route('app.orcamentos.update', $orcamento) }}" id="formOrcamento">
    @csrf
    @method('PUT')
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <div class="input-group" id="clienteBuscaGroup" style="{{ $orcamento->cliente ? 'display:none' : '' }}">
                        ...
                        <input type="hidden" name="cliente_id" id="clienteId" value="{{ $orcamento->cliente_id }}" required>
                        <div id="clienteSelecionado" class="mt-2" style="{{ $orcamento->cliente ? '' : 'display:none' }}">
                            <span class="badge bg-primary fs-6" id="clienteNome">{{ $orcamento->cliente->nome_razao_social ?? '' }}</span>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select name="vendedor_id" class="form-select">
                            ...
                            @foreach($vendedores as $v)
                                <option value="{{ $v->id }}" {{ $orcamento->vendedor_id == $v->id ? 'selected' : '' }}>
                                    {{ $v->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="validade_ate" class="form-control" value="{{ $orcamento->validade_ate?->format('Y-m-d') }}" required>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <textarea name="observacoes_internas" class="form-control" rows="3">{{ $orcamento->observacoes_internas }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Observacoes para o Cliente</label>
                        <textarea name="observacoes_externas" class="form-control" rows="3">{{ $orcamento->observacoes_externas }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-6">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-2">
                        <div class="col-6">
                            ...
                            <input type="number" name="desconto_percentual" id="descontoPerc" class="form-control form-control-sm" step="0.01" min="0" max="100" value="{{ $orcamento->desconto_percentual ?? 0 }}">
                        </div>
                        <div class="col-6">
                            <label class="form-label small">Desconto (R$)</label>
                            <input type="number" name="desconto_valor" id="descontoValor" class="form-control form-control-sm" step="0.01" min="0" value="{{ $orcamento->desconto_valor ?? 0 }}">
                        </div>
                    </div>
                    <hr>
            ...
            <div class="mt-3 text-end">
                <button type="submit" class="btn btn-primary btn-lg">
                    <i class="bi bi-check-lg me-1"></i> Atualizar Orcamento
                </button>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addItemRow(data = null) {
        ...
        row.innerHTML = `
            <td>
                <input type="text" class="form-control form-control-sm produto-busca" placeholder="Buscar produto..." data-index="${itemIndex}" autocomplete="off" value="${data ? data.descricao : ''}">
