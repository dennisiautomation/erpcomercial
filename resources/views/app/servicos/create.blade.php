...
@section('content')
<x-erp.page-header title="Novo Servico" icon="tools">
    <a href="{{ route('app.servicos.index') }}" class="btn btn-erp-outline"><i class="bi bi-arrow-left me-1"></i>Voltar</a>
</x-erp.page-header>

<form method="POST" action="{{ route('app.servicos.store') }}" class="erp-form">
    @csrf

    <x-erp.form-section title="Dados do Servico" icon="info-circle">
        <div class="row g-3">
            <div class="col-md-3">
                <label for="codigo" class="form-label fw-semibold">Codigo</label>
                <input type="text" name="codigo" id="codigo" class="form-control @error('codigo') is-invalid @enderror" value="{{ old('codigo') }}" placeholder="Ex: SRV001">
                @error('codigo') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            <div class="col-md-9">
                <label for="descricao" class="form-label fw-semibold">Descricao <span class="text-danger">*</span></label>
                <input type="text" name="descricao" id="descricao" class="form-control @error('descricao') is-invalid @enderror" value="{{ old('descricao') }}" required placeholder="Ex: Manutencao preventiva, Instalacao de software...">
    ...
    <x-erp.form-section title="Dados Fiscais" icon="file-earmark-text">
        <div class="row g-3">
            <div class="col-md-4">
                <label for="codigo_servico_municipal" class="form-label fw-semibold">Cod. Servico Municipal</label>
                <input type="text" name="codigo_servico_municipal" id="codigo_servico_municipal" class="form-control @error('codigo_servico_municipal') is-invalid @enderror" value="{{ old('codigo_servico_municipal') }}" placeholder="Ex: 14.01">
            ...
            <div class="col-md-4">
                ...
                <input type="text" name="cnae" id="cnae" class="form-control @error('cnae') is-invalid @enderror" value="{{ old('cnae') }}" placeholder="Ex: 6201-5/01">
    ...
    <div class="d-flex gap-2 mb-4">
        <button type="submit" class="btn btn-erp-primary"><i class="bi bi-check-lg me-1"></i>Salvar Servico</button>
        <a href="{{ route('app.servicos.index') }}" class="btn btn-erp-outline">Cancelar</a>
    </div>
</form>
@endsection
