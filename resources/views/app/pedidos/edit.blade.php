...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1"><i class="bi bi-pencil-square me-2"></i>Editar Pedido #{{ $pedido->numero }}</h4>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 small">
                <li class="breadcrumb-item"><a href="{{ route('app.dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ route('app.pedidos.index') }}">Pedidos</a></li>
                <li class="breadcrumb-item"><a href="{{ route('app.pedidos.show', $pedido) }}">#{{ $pedido->numero }}</a></li>
    ...
    <div class="row g-4">
                        ...
                        <div id="clienteSelecionado" class="mt-2" style="{{ $pedido->cliente ? '' : 'display:none' }}">
                            <div class="d-flex align-items-center bg-primary bg-opacity-10 rounded-3 p-2 ps-3">
                                ...
                                <div class="flex-grow-1">
                                    <div class="fw-semibold" id="clienteNome">{{ $pedido->cliente->nome_razao_social ?? '' }}</div>
                                    <small class="text-muted" id="clienteDoc">{{ $pedido->cliente->cpf_cnpj ?? '' }}</small>
                                </div>
        ...
        <div class="col-lg-4">
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-0">
                        <label class="form-label small fw-semibold">Condicao de Pagamento</label>
                        <select name="condicao_pagamento" class="form-select">
                            <option value="">Selecione...</option>
                            @foreach(['a_vista' => 'A Vista', '30_dias' => '30 Dias', '30_60_dias' => '30/60 Dias', '30_60_90_dias' => '30/60/90 Dias', 'cartao' => 'Cartao', 'boleto' => 'Boleto', 'pix' => 'PIX'] as $val => $label)
                        ...
                        <div class="mt-2">
                            ...
                            <select name="canal" class="form-select">
                                @foreach(\App\Enums\CanalVenda::opcoes() as $val => $label)
                                    <option value="{{ $val }}" {{ old('canal', $pedido->canal?->value ?? 'presencial') === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
        ...
        <div class="col-12">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
                    <span class="fw-semibold"><i class="bi bi-list-ul me-1"></i> Itens do Pedido</span>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <textarea name="observacoes_internas" class="form-control" rows="3"
                                  placeholder="Notas internas...">{{ $pedido->observacoes_internas }}</textarea>
                    </div>
                    <div class="mb-0">
                        ...
                        <textarea name="observacoes_externas" class="form-control" rows="3"
                                  placeholder="Instrucoes de entrega...">{{ $pedido->observacoes_externas }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-6">
                        ...
                        <div class="col-6">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="number" name="desconto_percentual" id="descontoPerc"
                                       class="form-control" step="0.01" min="0" max="100" value="{{ $pedido->desconto_percentual ?? 0 }}">
                        ...
                        <div class="col-6">
                            ...
                            <div class="input-group input-group-sm">
                                ...
                                <input type="number" name="desconto_valor" id="descontoValor"
                                       class="form-control" step="0.01" min="0" value="{{ $pedido->desconto_valor ?? 0 }}">
                            </div>
                        </div>
                    </div>
            ...
            <div class="mt-3 d-flex gap-2 justify-content-end">
                <a href="{{ route('app.pedidos.show', $pedido) }}" class="btn btn-outline-secondary btn-lg">Cancelar</a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addItemRow(data = null) {
        ...
        row.innerHTML = `
            ...
            <td class="text-end"><span class="item-total fw-bold text-nowrap">R$ 0,00</span></td>
    ...
    function bindRowEvents(row) {
                    ...
                    produtos.forEach(p => {
                        ...
                        item.innerHTML = `
                            <div class="d-flex justify-content-between align-items-center">
                                <div><span class="fw-semibold">${p.descricao}</span> <small class="text-muted ms-1">${p.codigo_interno ? '(' + p.codigo_interno + ')' : ''}</small></div>
        ...
        row.querySelector('.btn-remove-item').addEventListener('click', function() { row.remove(); renumberRows(); updateEmptyState(); calcularTotais(); });
    }

    function renumberRows() {
        document.querySelectorAll('#itensBody tr').forEach((row, i) => { row.querySelector('.item-num').textContent = i + 1; });
    }
    ...
    function calcularTotais() {
        ...
        document.getElementById('totalDisplay').textContent = 'R$ ' + Math.max(0, subtotal - descValor).toFixed(2).replace('.', ',');
    }
        ...
        clienteTimeout = setTimeout(() => {
            ...
            .then(clientes => {
                ...
                clientes.forEach(c => {
                    ...
                    item.href = '#';
                    item.className = 'list-group-item list-group-item-action py-2';
                    item.innerHTML = `<div class="fw-semibold">${c.nome_razao_social}</div><small class="text-muted">${c.cpf_cnpj || 'Sem documento'}</small>`;
                    item.addEventListener('click', function(e) {
                        e.preventDefault();
