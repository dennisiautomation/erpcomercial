...
class PdvController extends Controller
{
    public function index()
    {
        ...
        return view('app.pdv.index', compact('caixa', 'produtos'));
    }
    ...
    public function registrarVenda(Request $request)
    {
        ...
        $venda = DB::transaction(function () use ($request, $caixa) {
            ...
            $ultimoNumero = Venda::where('empresa_id', $empresaId)->max('numero');
            ...
            foreach ($pagamentos as $pgto) {
                ContaReceber::create([
                    ...
                    'valor'           => min($pgto['valor'], $total),
                    'valor_pago'      => min($pgto['valor'], $total),
                    'vencimento'      => now(),
                    'pago_em'         => now(),
            ...
            if ($vendedorId) {
                $percentualComissao = 5; // Default 5%, could be configurable
        ...
        $venda->load(['itens.produto', 'cliente', 'vendedor']);

        // Verificar configuração fiscal da unidade
        $empresaId = session('empresa_id');
        $unidadeId = session('unidade_id');
        ...
        if ($config && $config->emissao_fiscal_ativa && $config->tipo_cupom_pdv === 'fiscal') {
            ...
            try {
                ...
                Log::error('[PDV] Erro ao emitir NFC-e, gerando cupom não fiscal.', [
                    'venda_id' => $venda->id,
                    'error' => $e->getMessage(),
                ]);
                // Fallback para cupom não fiscal
                $cupomHtml = view('app.pdv.cupom-nao-fiscal', compact('venda'))->render();
            }
        } else {
            // Cupom não fiscal (recibo)
            $cupomHtml = view('app.pdv.cupom-nao-fiscal', compact('venda'))->render();
        }
